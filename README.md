# SQL Injection & JWT Authentication Lab
...
- **Secure Implementation**: Includes examples of how to fix these vulnerabilities using **Parameterized Queries**.
...
```
**The Attack:** 
Enter `admin'--` as the username. The query becomes:
...
```
**The Attack:**
Enter `hacker', 'password', 'admin')--` as the username. The query becomes:
`INSERT INTO users (username, password, role) VALUES ('hacker', 'password', 'admin')--', '...', 'user')`
This bypasses the default 'user' role and creates an 'admin' account.
...
### Windows (install.bat)
Run this script to install dependencies and start the server (requires Node.js and pnpm).
```batch
@echo off
...
---
**Disclaimer**: This project is for educational purposes only. Never use these vulnerable patterns in production applications.
